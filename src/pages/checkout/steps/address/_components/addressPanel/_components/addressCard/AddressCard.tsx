...
export function AddressCard({ address, selected, onSelect, onEdit, onDelete, deleting }: AddressCardProps) {
  const [confirmDelete, setConfirmDelete] = useState(false);

  return (
    <div
      className={`w-full rounded-xl border transition-all duration-200 overflow-hidden ${
        selected ? "border-blue-800 bg-blue-50 shadow-sm shadow-blue-100" : "border-blue-100 bg-white hover:border-blue-200"
      }`}
    >
      <button type="button" onClick={onSelect} className="w-full text-right flex items-start gap-3 p-3 sm:p-4">
        <div
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-bold text-gray-800 line-clamp-1">{address.receiverFullName}</p>
          <p className="text-xs text-gray-500 leading-relaxed mt-0.5 line-clamp-2">{address.receiverAddress}</p>
