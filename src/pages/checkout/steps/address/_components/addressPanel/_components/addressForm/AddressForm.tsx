  ...
  return (
    <form onSubmit={handleSubmit(config.onSave)} className="flex flex-col gap-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <AddressField label="نام و نام خانوادگی" error={errors.receiverFullName?.message} icon={IconUser}>
          <input
        ...
        <AddressField label="شماره موبایل" error={errors.receiverMobile?.message} icon={IconPhone}>
          <input
      ...
      <AddressField label="آدرس کامل" error={errors.receiverAddress?.message} icon={IconMapPin}>
        <input
      ...
      <AddressField label="کد پستی" error={errors.receiverZipCode?.message} icon={IconMail}>
        <input
          {...register("receiverZipCode", { required: "کد پستی الزامی است" })}
          placeholder="۱۲۳۴۵۶۷۸۹۰"
          disabled={loading}
          className={`${addressInputClass(!!errors.receiverZipCode)} sm:w-1/2`}
        />
      </AddressField>
      ...
      <div className="flex gap-2 flex-row-reverse pt-1">
        ...
        >
          {loading ? (
            ...
            <>
              <Check size={14} />
              <span>{config.mode === "edit" ? "ذخیره تغییرات" : "ذخیره آدرس"}</span>
            </>
          )}
        </button>
        {onCancel && (
