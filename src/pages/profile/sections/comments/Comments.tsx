...
export default function Comments() {
  ...
  const [editingComment, setEditingComment] = useState<IUserComments | null>(null);
  const [savingEdit, setSavingEdit] = useState(false);

  const [pendingDeleteComment, setPendingDeleteComment] = useState<IUserComments | null>(null);
  ...
  return (
    <div className="flex flex-col gap-3.5 sm:gap-5" dir="rtl">
      ...
      <div className="flex items-center gap-1.5 sm:gap-2">
        <IconMessage2 size={18} className="text-blue-800 sm:w-5 sm:h-5" />
        <h1 className="text-sm sm:text-lg font-bold text-blue-800">نظرات من</h1>
      </div>
      ...
      {loading.comments ? (
        ...
        <>
          <CommentsStatsStrip totalCount={comments.length} repliedCount={repliedCount} />
