  ...
  return (
    <div className="w-full min-w-0 bg-white border border-blue-100 rounded-xl sm:rounded-2xl p-3.5 sm:p-5 flex flex-col gap-2.5 sm:gap-3">
      ...
      <div className="flex items-start justify-between gap-2 sm:gap-3 min-w-0">
        <div className="flex flex-1 items-center gap-2.5 sm:gap-3 min-w-0">
          ...
          <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-lg sm:rounded-xl bg-blue-50 border border-blue-100 flex items-center justify-center flex-shrink-0">
            <IconPill size={16} className="text-blue-800 sm:w-[18px] sm:h-[18px]" />
          </div>
          <div className="flex flex-col items-start w-full">
            ...
            <Link to={`/plp/${encodeURIComponent(comment.categoryFullSlug)}`}>
              <p className="text-[10px] sm:text-xs text-gray-400 hover:text-gray-600 mt-0.5 transition-colors">{comment.categoryName}</p>
            </Link>
          </div>
        </div>

        <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
          <CommentCardMenu onEdit={onEdit} onDelete={onDelete} />
        </div>
      </div>
        ...
        <div className="w-full bg-emerald-50 border border-emerald-100 rounded-lg sm:rounded-xl px-3 sm:px-4 py-2.5 sm:py-3 flex flex-col gap-1 sm:gap-1.5">
          <div className="flex items-center justify-between gap-1.5 flex-wrap">
            <div className="flex items-center gap-1.5">
              <IconMessageCheck size={12} className="text-emerald-600 sm:w-[13px] sm:h-[13px]" />
              <span className="text-[10px] sm:text-xs font-semibold text-emerald-700">پاسخ فروشگاه</span>
            </div>
            {comment.replyDate && (
              <span className="text-[10px] sm:text-xs text-gray-400 sm:mr-auto flex items-center gap-1">
                <IconCalendar size={10} className="sm:w-[11px] sm:h-[11px]" />
                <span dir="ltr">{toPersianDigits(comment.replyDate)}</span>
              </span>
            )}
          </div>
          <p className="text-[10px] sm:text-xs text-gray-600 leading-5 sm:leading-relaxed">{comment.reply}</p>
        </div>
      )}
      ...
      <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <CommentVoteStats likeCount={comment.likeCount} dislikeCount={comment.dislikeCount} />
      </div>
    </div>
  );
}