...
const daysSince = (dateStr: string): number => {
    try {
        const gregorianDate = convertDate(dateStr.split(" ")[0], {
            from: "jalali",
            to: "gregorian",
        });
    ...
    if (done) {
        return (
            <div className="flex items-center gap-2 bg-green-50 border border-green-100 rounded-lg sm:rounded-xl px-3 sm:px-4 py-2.5 sm:py-3">
                <IconCircleCheck
                    size={14}
                    className="text-green-600 flex-shrink-0 sm:w-[15px] sm:h-[15px]"
                />
                <p className="text-[11px] sm:text-xs font-medium text-green-700">
    ...
    if (expired) {
        return (
            <div className="flex items-start gap-2 bg-gray-50 border border-gray-200 rounded-lg sm:rounded-xl px-3 sm:px-4 py-2.5 sm:py-3">
                <IconLock
                    size={14}
                    className="text-gray-400 flex-shrink-0 mt-0.5 sm:w-[15px] sm:h-[15px]"
                />
                <div>
                    <p className="text-[11px] sm:text-xs font-semibold text-gray-500">
                        مهلت مرجوعی پایان یافته
                    </p>
                    <p className="text-[10px] sm:text-xs text-gray-400 mt-0.5">
                        {toPersianDigits(days)} روز از تحویل گذشته — مهلت مرجوعی
                        ۷ روز است
                    </p>
                </div>
            </div>
            ...
            {open && (
                <div className="flex flex-col gap-2.5 sm:gap-3 bg-rose-50 border border-rose-100 rounded-xl sm:rounded-2xl p-3 sm:p-4">
                    <div className="flex items-center gap-2">
                        <IconAlertTriangle
                            size={13}
                            className="text-rose-500 flex-shrink-0 sm:w-3.5 sm:h-3.5"
                        />
                        <p className="text-[11px] sm:text-xs font-semibold text-rose-700">
                            دلیل مرجوعی را بنویسید
                        </p>
                    </div>
                    ...
                    <div className="flex gap-2">
                        ...
                        >
                            {loading.requestReturn ? (
                                <IconLoader2
                                    size={12}
                                    className="animate-spin sm:w-[13px] sm:h-[13px]"
                                />
                            ) : (
                                <IconRotateClockwise
                                    size={12}
