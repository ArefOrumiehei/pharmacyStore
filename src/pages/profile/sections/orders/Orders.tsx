    ...
    <div className="flex flex-col gap-3.5 sm:gap-5">
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="min-w-0">
          <h1 className="text-base sm:text-xl font-bold text-blue-800">سفارش‌های من</h1>
          {!loading.orders && (
            <p className="text-xs sm:text-sm text-gray-400 mt-0.5">{toPersianDigits(orders?.length)} سفارش ثبت شده</p>
          )}
        </div>
        {loading.orders && (
      ...
      {!loading.orders && orders?.length > 0 && (
        <OrderStatusFilter statuses={statuses} activeStatus={filterStatus} onChange={setFilterStatus} />
      )}
