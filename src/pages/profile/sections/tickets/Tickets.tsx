...
/* ─────────────────────────────────────────
   TICKET DETAIL (document style)
───────────────────────────────────────── */
function TicketDetail({ ticket, onBack }: { ticket: ITicket; onBack: () => void }) {
  const status = getStatus(ticket);
...
export default function Tickets() {
  const {
    userTickets, loading,
    fetchUserTickets,
    selectedTicket, fetchTicketDetails, clearSelectedTicket,
  } = useUserStore();
  ...
  const handleSelectTicket = (ticket: ITicket) => {
    fetchTicketDetails(ticket.trackingCode);
  };
  ...
  if (selectedTicket) {
    return (
      <div className="flex flex-col gap-5" dir="rtl">
        <TicketDetail ticket={selectedTicket} onBack={clearSelectedTicket} />
      </div>
    );
  }
