...
export default function RecentOrdersCard({ orders, loading }: RecentOrdersCardProps) {
    ...
    <div className="bg-white border border-blue-100 rounded-xl sm:rounded-2xl p-3.5 sm:p-6">
      {hasOrders && (
        <div className="flex items-center justify-between mb-3 sm:mb-4">
          ...
          >
            مشاهده همه
            <IconChevronLeft size={12} className="sm:w-[13px] sm:h-[13px]" />
          </Link>
        </div>
      )}
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center gap-2 py-6 sm:py-8 text-center">
          <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl sm:rounded-2xl bg-blue-50 flex items-center justify-center">
            <IconShoppingBag size={18} className="text-blue-300 sm:w-[22px] sm:h-[22px]" />
          </div>
          <p className="text-xs sm:text-sm text-gray-400">هنوز سفارشی ثبت نشده است</p>
        </div>
      ) : (
