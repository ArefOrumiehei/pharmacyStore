    ...
    <div className="w-full max-w-full bg-white border border-blue-100 rounded-xl sm:rounded-2xl p-3.5 sm:p-5 flex flex-col gap-3 sm:gap-4 overflow-hidden">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 sm:gap-3">
        <div className="flex flex-col gap-1 sm:gap-1.5 min-w-0">
          <label className="text-[10px] sm:text-xs font-medium text-gray-600">نام گیرنده</label>
          <input
            {...register("receiverFullName", { required: "نام الزامی است" })}
            placeholder="نام کامل گیرنده"
            className={inputClass(!!errors.receiverFullName)}
          />
          {errors.receiverFullName && <p className="text-rose-500 text-[10px] sm:text-xs">{errors.receiverFullName.message}</p>}
        </div>

        <div className="flex flex-col gap-1 sm:gap-1.5 min-w-0">
          <label className="text-[10px] sm:text-xs font-medium text-gray-600">شماره موبایل</label>
          <input
            {...register("receiverMobile", { required: "موبایل الزامی است" })}
            placeholder="۰۹۱۲۳۴۵۶۷۸۹"
            className={inputClass(!!errors.receiverMobile)}
          />
          {errors.receiverMobile && <p className="text-rose-500 text-[10px] sm:text-xs">{errors.receiverMobile.message}</p>}
        </div>

        <div className="flex flex-col gap-1 sm:gap-1.5 sm:col-span-2 min-w-0">
          <label className="text-[10px] sm:text-xs font-medium text-gray-600">آدرس کامل</label>
          <input
          ...
          />
          {errors.receiverAddress && <p className="text-rose-500 text-[10px] sm:text-xs">{errors.receiverAddress.message}</p>}
        </div>

        <div className="flex flex-col gap-1 sm:gap-1.5 min-w-0">
          <label className="text-[10px] sm:text-xs font-medium text-gray-600">کد پستی</label>
          <input
            {...register("receiverZipCode", { required: "کد پستی الزامی است" })}
            placeholder="۱۲۳۴۵۶۷۸۹۰"
            className={inputClass(!!errors.receiverZipCode)}
          />
          {errors.receiverZipCode && <p className="text-rose-500 text-[10px] sm:text-xs">{errors.receiverZipCode.message}</p>}
        </div>
      </div>
      ...
      <div className="flex flex-col-reverse sm:flex-row gap-2 justify-end">
        ...
        >
          <IconX size={13} className="sm:w-[15px] sm:h-[15px]" /> انصراف
        </button>
        <button
          type="button"
        ...
        >
          {saving ? <IconLoader2 size={13} className="animate-spin sm:w-[15px] sm:h-[15px]" /> : <IconCheck size={13} className="sm:w-[15px] sm:h-[15px]" />}
          ذخیره آدرس
        </button>
      </div>
    </div>
