...
export default function CommentForm({
  message, rate, isEditing, onMessageChange, onRateChange, onSubmit, onCancel,
}: CommentFormProps) {
  return (
    <div className="bg-white rounded-xl sm:rounded-2xl border border-blue-100 p-4 sm:p-6 flex flex-col gap-3 sm:gap-4">
      <SectionTitle>{isEditing ? "ویرایش نظر" : "ثبت نظر جدید"}</SectionTitle>
      ...
      <div className="flex gap-2">
        {isEditing && (
          <button onClick={onCancel} className="flex-1 py-2 sm:py-2.5 rounded-lg sm:rounded-xl border border-gray-200 text-xs sm:text-sm text-gray-500 hover:bg-gray-50 transition-all">
            انصراف
          </button>
        )}
        <button
